...
function createCookie(newCookie) {
    const {name, value, expires} = newCookie;

    if (doesCookieExist(name)) {
        createToast({name, state: "modifié", color: "orangered"});
    } else {
        createToast({name, state: "créé", color: "green"});
    }
...
function doesCookieExist(name) {
    ...
    return onlyCookiesName.includes(encodeURIComponent(name));
}
